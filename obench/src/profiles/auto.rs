...
pub async fn run(cli: &Cli, tgt: Target, scope: Scope, seeded: &SeededRun, proxy_base: &str) -> Result<i32> {
    ...
    for &conc in STEPS {
        ...
        crate::engine::load::run_closed_loop(
            client, make_req,
            RunConfig { conc, duration_s: STEP_SECS, warmup_s: 2 },
            stop, stats.clone(),
        ).await;
        ...
        let latest = StepResult {
            conc, req_per_s: sum.req_per_s, error_rate: sum.error_rate,
            p99_ttfb_ms: sum.p99_ttfb_ms, max_queued: 0,
        };
        println!("  step conc={conc}: {:.0} req/s  err {:.2}%  p99 {}ms", latest.req_per_s, latest.error_rate * 100.0, latest.p99_ttfb_ms);

        match evaluate(&history, &latest, &cfg) {
            Decision::Continue => { sustainable = conc; history.push(latest); }
            Decision::Stop { last_clean_conc, reason } => {
    ...
    if sustainable == 0 {
        println!("\nno sustainable level found — even the lowest step ({}) exceeded the error ceiling", STEPS[0]);
    } else {
        println!("\nsustainable ceiling for this deployment: conc={sustainable}");
    }
    report::write_meta("auto", &serde_json::json!({
