...
pub async fn run_headless(cli: &Cli, tgt: Target, profile: Profile, scope: Scope) -> Result<i32> {
    ...
    let seeded: SeededRun = match tgt {
        Target::Fixture => {
            let api_base = std::env::var("BENCHMARK_API_BASE").unwrap_or_else(|_| FIXTURE_API_BASE_DEFAULT.to_string());
    ...
    let make_req = {
        ...
        move || -> ChatRequest {
            ...
            let tweights: Vec<u32> = seeded_arc.tenants.iter().map(|t| t.traffic_share.max(1)).collect();
            ...
            let (model, out_tokens, stream) = if seeded_arc.models.iter().all(|m| m.starts_with("obench-")) {
                let cands: Vec<&fleet::TrafficType> = fleet::FIXTURE_TRAFFIC.iter()
                    .filter(|t| seeded_arc.models.iter().any(|m| m == t.model)).collect();
                if cands.is_empty() {
                    ...
                    (tt.model.to_string(), if tt.output_tokens > 0 { tt.output_tokens } else { plan_out }, tt.kind == TrafficKind::ChatStream)
                }
            } else {
                let mi = rng.gen_range(0..seeded_arc.models.len());
                (seeded_arc.models[mi].clone(), plan_out, plan_stream)
            };
            ChatRequest { proxy_base: proxy_base.clone(), key: tenant.key.clone(), model, input_tokens, output_tokens: out_tokens, stream }
        }
    };
    ...
    // Sample fairshare into the timeline for heavy/auto/live.
        ...
        tokio::spawn(async move {
            ...
            while !stop.load(Ordering::Relaxed) {
                if let Ok(live) = a.fairshare_live().await {
                    let _ = report::append_timeline(&pname, &serde_json::json!({
    ...
    report::write_meta(&profile_name, &serde_json::json!({
        "target": format!("{tgt:?}"), "profile": profile_name, "scope": format!("{scope:?}"),
    ...
    Ok(match summary.verdict { Verdict::Pass => 0, Verdict::Fail(_) => 1 })
}
