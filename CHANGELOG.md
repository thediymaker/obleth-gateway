...
- **The global in-flight ceiling defaults to 4096.** With per-model pools doing the limiting, `OBLETH_GLOBAL_MAX_IN_FLIGHT` is a per-replica safety cap, and the earlier default of 1024 sat below the sum of pool sizes on a fleet of more than 32 default-sized models, which logged the start-up warning and let the ceiling bind first. The code default, the Helm value `obleth.globalMaxInFlight`, and the compose file now use 4096. A deployment that set its own value keeps it.
