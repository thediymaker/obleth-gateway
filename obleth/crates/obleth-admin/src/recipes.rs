...
fn view(r: obleth_store::Recipe) -> RecipeView {
    RecipeView { id: r.id, name: r.name, body: r.body, author: r.author }
}
...
pub async fn list_recipes(State(state): State<AdminState>) -> Result<Json<Vec<RecipeView>>> {
    Ok(Json(state.store.list_recipes().await?.into_iter().map(view).collect()))
}
...
pub async fn create_recipe(
    ...
    let r = state.store.upsert_recipe(obleth_store::UpsertRecipe {
        id: None, name: b.name, body: b.body, author: b.author,
    }).await?;
    Ok(Json(view(r)))
}
...
pub async fn update_recipe(
    ...
    if b.name.trim().is_empty() { return Err(AdminError::BadRequest("name required".into())); }
    let r = state.store.upsert_recipe(obleth_store::UpsertRecipe {
        id: Some(id), name: b.name, body: b.body, author: b.author,
    }).await?;
    Ok(Json(view(r)))
}
