...
pub trait OblethClient: Send + Sync {
    ...
    async fn create_endpoint(&self, model_id: Uuid, name: &str, api_base: &str) -> anyhow::Result<Uuid>;
...
fn replica_views_from_json(rows: &serde_json::Value) -> Vec<ReplicaView> {
    ...
    for r in rows.as_array().map(|a| a.as_slice()).unwrap_or_default() {
        let id: Uuid = match r.get("id").and_then(|x| x.as_str()).and_then(|s| s.parse().ok()) {
            Some(id) => id,
            None => {
        ...
        let model_id: Uuid = match r.get("model_id").and_then(|x| x.as_str()).and_then(|s| s.parse().ok()) {
            Some(m) => m,
            None => {
        ...
        out.push(ReplicaView {
            id,
            model_id,
            slurm_job_id: r.get("slurm_job_id").and_then(|x| x.as_str()).unwrap_or_default().to_string(),
...
impl HttpObleth {
    ...
    fn req(&self, m: reqwest::Method, path: &str) -> reqwest::RequestBuilder {
        self.http.request(m, format!("{}{path}", self.base)).bearer_auth(&self.token)
    }
}
...
impl OblethClient for HttpObleth {
    async fn list_managed_models(&self) -> anyhow::Result<Vec<ManagedModelSpec>> {
        ...
        let models: serde_json::Value = self.req(reqwest::Method::GET, "/models").send().await?.json().await?;
        let mut out = Vec::new();
        for m in models.as_array().cloned().unwrap_or_default() {
            ...
            if let Some(s) = spec { if s.enabled { out.push(s); } }
        }
        Ok(out)
    }
    ...
    async fn model_name(&self, model_id: Uuid) -> anyhow::Result<String> {
        let m: serde_json::Value = self
            .req(reqwest::Method::GET, &format!("/models/{model_id}")).send().await?.json().await?;
        Ok(m.get("model_name").and_then(|x| x.as_str()).unwrap_or_default().to_string())
    }

    async fn list_all_replicas(&self) -> anyhow::Result<Vec<ReplicaView>> {
        let rows: serde_json::Value = self
            .req(reqwest::Method::GET, "/replicas").send().await?.json().await?;
    ...
    async fn patch_replica(
        ...
        if let Some(s) = state { body.insert("state".into(), serde_json::json!(s)); }
        if let Some(n) = nodes { body.insert("nodes".into(), serde_json::json!(n)); }
        if let Some(e) = endpoint_id { body.insert("endpoint_id".into(), serde_json::json!(e.to_string())); }
        if let Some(m) = message { body.insert("message".into(), serde_json::json!(m)); }
        self.req(reqwest::Method::PATCH, &format!("/replicas/{replica_id}"))
            .json(&serde_json::Value::Object(body)).send().await?.error_for_status()?;
        Ok(())
    }
    ...
    async fn list_endpoints(&self, model_id: Uuid) -> anyhow::Result<Vec<(Uuid, String)>> {
        let rows: serde_json::Value = self
            .req(reqwest::Method::GET, &format!("/models/{model_id}/endpoints"))
            .send().await?.json().await?;
        let mut out = Vec::new();
        for e in rows.as_array().cloned().unwrap_or_default() {
            let id = e.get("id").and_then(|x| x.as_str()).and_then(|s| s.parse().ok());
            let name = e.get("name").and_then(|x| x.as_str()).unwrap_or_default().to_string();
            if let Some(id) = id { out.push((id, name)); }
        }
        Ok(out)
    }

    async fn delete_endpoint(&self, model_id: Uuid, endpoint_id: Uuid) -> anyhow::Result<()> {
        self.req(reqwest::Method::DELETE, &format!("/models/{model_id}/endpoints/{endpoint_id}"))
            .send().await?.error_for_status()?;
        Ok(())
    }
}
...
impl OblethClient for MockObleth {
    ...
    async fn patch_replica(
        &self, replica_id: Uuid, state: Option<&str>, _nodes: Option<&str>,
        _endpoint_id: Option<Uuid>, _message: Option<&str>,
    ) -> anyhow::Result<()> {
        self.patched.lock().unwrap().push((replica_id, state.map(|s| s.to_string())));
        Ok(())
    }
    async fn delete_replica(&self, replica_id: Uuid) -> anyhow::Result<()> {
    ...
    async fn create_endpoint(&self, model_id: Uuid, name: &str, api_base: &str) -> anyhow::Result<Uuid> {
        self.created_endpoints.lock().unwrap().push((model_id, name.to_string(), api_base.to_string()));
        Ok(Uuid::new_v4())
    }
